...
# Trying out new box plot types using ggplot 2 , This doesn't make any senes.
...
#this method is super expensive for this data set,may it gives the vector size of 827 mb)
# output$Mosaic <- renderPlot({
#   formula <- as.formula(~Y+Price+Duration+Speed)
#   mosaic(formula, data = choicesB,
#          main = "Outof the blue", shade = TRUE, legend = TRUE)
#   
#   browser()
...
#     data <- data[data$Durationl == input$Duration,]
#   }
#   if (input$Speed != "All") {
#     data <- data[data$Speed == input$Speed,]
#   }
#   data
# }))
